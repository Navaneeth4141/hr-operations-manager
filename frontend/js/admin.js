...
async function loadPasswordRequests() {
  ...
  try {
    const res = await apiRequest('/admin/password-requests');
    ...
    tbody.innerHTML = requests.map(req => `
      <tr>
        <td>${req.company ? req.company.name : 'Unknown'}</td>
...
async function approvePasswordRequest(id) {
  ...
  try {
    const res = await apiRequest(`/admin/password-requests/${id}/approve`, { method: 'PUT' });
...
async function rejectPasswordRequest(id) {
  ...
  try {
    const res = await apiRequest(`/admin/password-requests/${id}/reject`, { method: 'PUT' });
    const data = await res.json();
    if (res.ok) {
      showAlert(data.message, true);
      loadPasswordRequests();
